...
data.loadData().then(async () => {
  ...
  model.add(tf.layers.dense({ inputShape: [data.numFeatures], units: 100, activation: 'sigmoid' }));
  ...
  tf.tidy(() => {
    ...
    ui.updateStatus(
      `최종 테스트 세트 손실: ${lastTrainLog.loss.toFixed(4)} 정확도: ${lastTrainLog.acc.toFixed(
        4,
      )}\n` +
